...
function generateListTypesSql(listTypes: ListTypeData[]): string {
  const values = listTypes
    .map((lt) => {
      ...
      return `  (${sqlStr(lt.name)}, ${sqlStr(lt.englishFriendlyName)}, ${sqlStr(lt.welshFriendlyName)}, ${sqlStr(shortened)}, ${sqlStr(url)}, ${sqlStrOrNull(lt.defaultSensitivity)}, ${sqlStr(formatProvenance(lt.provenance))}, ${sqlBool(lt.isNonStrategic)}, NOW(), NOW())`;
    })
    .join(",\n");
