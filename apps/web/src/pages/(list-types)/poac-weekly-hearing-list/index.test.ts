...
const MOCK_JSON_DATA = [
  {
    ...
    caseReferenceNumber: "POAC/00001/2026",
...
describe("POAC Weekly Hearing List page controller", () => {
  ...
  describe("GET handler", () => {
    it("should render the list successfully with valid data", async () => {
      ...
      expect(renderSiacPoacPaacData).toHaveBeenCalledWith(MOCK_JSON_DATA, expect.objectContaining({ courtName: "Proscribed Organisations Appeal Commission" }));
      expect(res.render).toHaveBeenCalledWith("poac-weekly-hearing-list", expect.objectContaining({ dataSource: "Manual Upload" }));
    });
    ...
    it("should render in Welsh when locale is cy", async () => {
      ...
      expect(res.render).toHaveBeenCalledWith("poac-weekly-hearing-list", expect.objectContaining({ dataSource: "Lanlwytho â Llaw" }));
    });
  });
});
