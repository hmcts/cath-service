...
describe("crown-firm-list controller", () => {
  ...
  it("should return 400 when JSON validation fails", async () => {
    ...
    expect(validateCrownFirmList).toHaveBeenCalled();
    expect(consoleErrorSpy).toHaveBeenCalledWith("[crown-firm-list] Validation errors:", ["Validation error"]);
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.render).toHaveBeenCalledWith("errors/common", expect.any(Object));
  });

  it("should successfully render firm list in English", async () => {
    req.query = { artefactId: "test-artefact-123" };
    ...
    expect(renderCall[1]).toHaveProperty("groupedListData");
    expect(renderCall[1]).toHaveProperty("t");
  });

  it("should successfully render firm list in Welsh", async () => {
    req.query = { artefactId: "test-artefact-123" };
    ...
    expect(res.render).toHaveBeenCalledWith("crown-firm-list", expect.any(Object));
  });
  ...
  it("should return 500 on unexpected error", async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith("[crown-firm-list] Unexpected error:", expect.any(Error));
