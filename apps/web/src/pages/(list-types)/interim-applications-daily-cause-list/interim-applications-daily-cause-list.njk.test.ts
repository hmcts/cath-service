...
import { interimApplicationsDailyCauseListCy as cy, interimApplicationsDailyCauseListEn as en } from "@hmcts/interim-applications-daily-cause-list";
...
const TEMPLATE = "interim-applications-daily-cause-list.njk";
...
function buildImportantInfo(locale: typeof en | typeof cy = en) {
  return {
    ...
    thirdParagraph: locale.openJusticeIndividualList
  };
}
...
describe("interim-applications-daily-cause-list template", () => {
  ...
  describe("Important information section", () => {
    it("should render the editable paragraph with the judge name and email, plus the static paragraph", () => {
      ...
      expect(detailsText).toContain(en.openJusticeIndividualList);
    });

    it("should render the static fallback wording when no judge details are supplied", () => {
      const { $ } = renderPage({
        importantInfo: {
          editableParagraph: en.openJusticeFallback,
          staticParagraph: en.openJusticeMessage3,
          thirdParagraph: en.openJusticeIndividualList
        }
      });
