import { renderCauseListData, validateFamilyDailyCauseList } from "@hmcts/family-daily-cause-list";
...
describe("family-daily-cause-list controller", () => {
  ...
  it("should return 400 when JSON validation fails", async () => {
    ...
    expect(validateFamilyDailyCauseList).toHaveBeenCalled();
    expect(consoleErrorSpy).toHaveBeenCalledWith("[family-daily-cause-list] Validation errors:", ["Validation error"]);
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.render).toHaveBeenCalledWith("errors/common", expect.any(Object));
  });
  ...
  it("should use raw provenance when label not found", async () => {
    ...
    expect(res.render).toHaveBeenCalledWith("family-daily-cause-list", expect.objectContaining({ dataSource: "UNKNOWN_PROVENANCE" }));
  });
  ...
  it("should return 500 on unexpected error", async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith("[family-daily-cause-list] Unexpected error:", expect.any(Error));
