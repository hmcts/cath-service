...
import { renderMagistratesAdultList, validateMagistratesAdultCourtListDaily } from "@hmcts/magistrates-adult-court-list-daily";
...
vi.mock("@hmcts/magistrates-adult-court-list-daily");
...
describe("magistrates-adult-court-list-daily controller", () => {
  ...
  it("should pass undefined listType when dbListType is not found", async () => {
    ...
    vi.mocked(validateMagistratesAdultCourtListDaily).mockReturnValue({ isValid: true, errors: [] } as any);
  ...
  it("should return 400 when JSON validation fails", async () => {
    ...
    vi.mocked(getArtefactById).mockResolvedValue(mockArtefact);
    vi.mocked(readFile).mockResolvedValue(JSON.stringify(mockJsonData));
    vi.mocked(validateMagistratesAdultCourtListDaily).mockReturnValue({ isValid: false, errors: ["Validation error"] } as any);

    await GET(req as Request, res as Response);

    expect(validateMagistratesAdultCourtListDaily).toHaveBeenCalled();
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.render).toHaveBeenCalledWith("errors/common", expect.any(Object));
  });
  ...
  it("should successfully render the list in English", async () => {
    ...
    expect(renderCall[0]).toBe("magistrates-adult-court-list-daily");
  ...
  it("should successfully render the list in Welsh", async () => {
    ...
    expect(res.render).toHaveBeenCalledWith("magistrates-adult-court-list-daily", expect.any(Object));
  });
  ...
  it("should use provenance label for data source", async () => {
    ...
    expect(res.render).toHaveBeenCalledWith("magistrates-adult-court-list-daily", expect.objectContaining({ dataSource: "Manual Upload" }));
  });
