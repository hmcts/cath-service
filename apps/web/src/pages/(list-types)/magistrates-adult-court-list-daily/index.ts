...
import {
  ...
  validateMagistratesAdultCourtListDaily
...
export const GET = async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      console.error(`[magistrates-adult-court-list-daily] Error reading JSON file at ${jsonFilePath}:`, error);
      return res.status(404).render("errors/common", {
        en,
        cy,
        errorTitle: t.errorTitle,
    ...
    const validationResult = validateMagistratesAdultCourtListDaily(jsonData);
    if (!validationResult.isValid) {
      console.error("[magistrates-adult-court-list-daily] Validation errors:", validationResult.errors);
      return res.status(400).render("errors/common", {
        en,
        cy,
        errorTitle: t.errorTitle,
    ...
    res.render("magistrates-adult-court-list-daily", {
      en,
      cy,
      title: t.title,
      header,
    ...
    console.error("[magistrates-adult-court-list-daily] Unexpected error:", error);
