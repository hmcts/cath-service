...
describe("FTT RPT Northern Weekly Hearing List page controller", () => {
  ...
  describe("GET handler", () => {
    it("should render the list successfully with valid data", async () => {
      ...
      expect(renderFttRptData).toHaveBeenCalledWith(
        mockJsonData,
        expect.objectContaining({ courtName: "First-tier Tribunal (Residential and Property Tribunal) Northern Region" })
      );
      const renderCall = vi.mocked(res.render).mock.calls[0];
      expect(renderCall[0]).toBe("ftt-rpt-northern-weekly-hearing-list");
