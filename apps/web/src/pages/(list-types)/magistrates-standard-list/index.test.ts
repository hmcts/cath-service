...
import { renderMagistratesListData, validateMagistratesStandardList } from "@hmcts/magistrates-standard-list";
...
describe("magistrates-standard-list controller", () => {
  ...
  describe("GET", () => {
    ...
    it("should successfully render in English", async () => {
      ...
      vi.mocked(validateMagistratesStandardList).mockReturnValue({ isValid: true, errors: [] } as any);
    ...
    it("should use provenance label for data source", async () => {
      ...
      expect(res.render).toHaveBeenCalledWith("magistrates-standard-list", expect.objectContaining({ dataSource: "Manual Upload" }));
    });
    ...
    it("should use raw provenance when label not found", async () => {
      ...
      expect(res.render).toHaveBeenCalledWith("magistrates-standard-list", expect.objectContaining({ dataSource: "UNKNOWN_PROVENANCE" }));
    });
  });
});
