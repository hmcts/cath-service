...
vi.mock("@hmcts/utiac-jr-london-daily-hearing-list", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@hmcts/utiac-jr-london-daily-hearing-list")>();
  return {
    ...actual,
...
import { renderUtiacJrLondonDailyHearingListData } from "@hmcts/utiac-jr-london-daily-hearing-list";
import { GET } from "./index.js";

describe("UTIAC JR London Daily Hearing List page controller", () => {
  ...
  describe("GET handler", () => {
    it("should render the list successfully with valid data", async () => {
      ...
      expect(renderCall[0]).toBe("utiac-jr-london-daily-hearing-list");
    ...
    it("should pass displayFrom to renderer", async () => {
      ...
      const mockRenderedData = {
        header: { listTitle: "UTIAC JR London", listForDate: "15 January 2026", lastUpdatedDate: "14 January 2026", lastUpdatedTime: "12pm" },
        hearings: []
      };
      ...
      // Assert
      expect(renderUtiacJrLondonDailyHearingListData).toHaveBeenCalledWith([], expect.objectContaining({ displayFrom }));
    });
    ...
    it("should use Welsh locale when specified", async () => {
      ...
      // Assert
      expect(renderUtiacJrLondonDailyHearingListData).toHaveBeenCalledWith([], expect.objectContaining({ locale: "cy" }));
    });
  });
});
