...
import { renderMagistratesPublicList, validateMagistratesPublicAdultCourtListDaily } from "@hmcts/magistrates-public-adult-court-list-daily";
...
vi.mock("@hmcts/magistrates-public-adult-court-list-daily");
...
describe("magistrates-public-adult-court-list-daily controller", () => {
  ...
  it("should pass undefined listType when dbListType is not found", async () => {
    ...
    vi.mocked(validateMagistratesPublicAdultCourtListDaily).mockReturnValue({ isValid: true, errors: [] } as any);
  ...
  it("should return 400 when JSON validation fails", async () => {
    ...
    vi.mocked(getArtefactById).mockResolvedValue(mockArtefact);
    vi.mocked(readFile).mockResolvedValue(JSON.stringify(mockJsonData));
    vi.mocked(validateMagistratesPublicAdultCourtListDaily).mockReturnValue({ isValid: false, errors: ["Validation error"] } as any);

    await GET(req as Request, res as Response);

    expect(validateMagistratesPublicAdultCourtListDaily).toHaveBeenCalled();
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.render).toHaveBeenCalledWith("errors/common", expect.any(Object));
  });
  ...
  it("should successfully render the list in English", async () => {
    ...
    expect(renderCall[0]).toBe("magistrates-public-adult-court-list-daily");
  ...
  it("should successfully render the list in Welsh", async () => {
    ...
    expect(res.render).toHaveBeenCalledWith("magistrates-public-adult-court-list-daily", expect.any(Object));
  });
  ...
  it("should use provenance label for data source", async () => {
    ...
    expect(res.render).toHaveBeenCalledWith("magistrates-public-adult-court-list-daily", expect.objectContaining({ dataSource: "Manual Upload" }));
  });
  ...
  it("should return 500 on unexpected error", async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith("[magistrates-public-adult-court-list-daily] Unexpected error:", expect.any(Error));
