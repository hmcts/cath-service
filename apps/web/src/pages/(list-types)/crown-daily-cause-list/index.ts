...
import { crownDailyListCy as cy, crownDailyListEn as en, renderCrownDailyListData, validateCrownDailyList } from "@hmcts/crown-daily-list";
...
export const GET = async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      console.error(`[crown-daily-list] Error reading JSON file at ${jsonFilePath}:`, error);
      return res.status(404).render("errors/common", {
        en,
        cy,
        errorTitle: t.errorTitle,
    ...
    if (!validationResult.isValid) {
      console.error("[crown-daily-list] Validation errors:", validationResult.errors);
      return res.status(400).render("errors/common", {
        en,
        cy,
        errorTitle: t.errorTitle,
    ...
    res.render("crown-daily-cause-list", {
      en,
      cy,
      title: t.title,
      header,
