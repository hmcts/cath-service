...
import { renderMagistratesPublicList, validateMagistratesPublicAdultCourtListFuture } from "@hmcts/magistrates-public-adult-court-list-future";
...
vi.mock("@hmcts/magistrates-public-adult-court-list-future");

describe("magistrates-public-adult-court-list-future controller", () => {
  ...
  it("should pass undefined listType when dbListType is not found", async () => {
    ...
    vi.mocked(validateMagistratesPublicAdultCourtListFuture).mockReturnValue({ isValid: true, errors: [] } as any);
    vi.mocked(renderMagistratesPublicList).mockResolvedValue(mockRenderedData);
  ...
  it("should return 400 when JSON validation fails", async () => {
    ...
    vi.mocked(getArtefactById).mockResolvedValue(mockArtefact);
    vi.mocked(readFile).mockResolvedValue(JSON.stringify(mockJsonData));
    vi.mocked(validateMagistratesPublicAdultCourtListFuture).mockReturnValue({ isValid: false, errors: ["Validation error"] } as any);

    await GET(req as Request, res as Response);

    expect(validateMagistratesPublicAdultCourtListFuture).toHaveBeenCalled();
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.render).toHaveBeenCalledWith("errors/common", expect.any(Object));
  });
  ...
  it("should successfully render the list in English", async () => {
    ...
    expect(renderMagistratesPublicList).toHaveBeenCalledWith(mockJsonData, {
      locationId: "1",
      contentDate: mockArtefact.contentDate,
      locale: "en"
    });
    const renderCall = vi.mocked(res.render).mock.calls[0];
    expect(renderCall[0]).toBe("magistrates-public-adult-court-list-future");
  ...
  it("should successfully render the list in Welsh", async () => {
    ...
    expect(res.render).toHaveBeenCalledWith("magistrates-public-adult-court-list-future", expect.any(Object));
  });
  ...
  it("should use provenance label for data source", async () => {
    ...
    expect(res.render).toHaveBeenCalledWith("magistrates-public-adult-court-list-future", expect.objectContaining({ dataSource: "Manual Upload" }));
  });
  ...
  it("should return 500 on unexpected error", async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith("[magistrates-public-adult-court-list-future] Unexpected error:", expect.any(Error));
