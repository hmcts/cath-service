...
import {
  magistratesPublicAdultCourtListFutureCy as cy,
  magistratesPublicAdultCourtListFutureEn as en,
  renderMagistratesPublicList,
  validateMagistratesPublicAdultCourtListFuture
...
export const GET = async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      console.error(`[magistrates-public-adult-court-list-future] Error reading JSON file at ${jsonFilePath}:`, error);
      return res.status(404).render("errors/common", {
        en,
        cy,
        errorTitle: t.errorTitle,
    ...
    const validationResult = validateMagistratesPublicAdultCourtListFuture(jsonData);
    if (!validationResult.isValid) {
      console.error("[magistrates-public-adult-court-list-future] Validation errors:", validationResult.errors);
      return res.status(400).render("errors/common", {
        en,
        cy,
        errorTitle: t.errorTitle,
    ...
    const { header, openJustice, listData } = await renderMagistratesPublicList(jsonData, {
    ...
    res.render("magistrates-public-adult-court-list-future", {
      en,
      cy,
      title: t.title,
      header,
    ...
    console.error("[magistrates-public-adult-court-list-future] Unexpected error:", error);
