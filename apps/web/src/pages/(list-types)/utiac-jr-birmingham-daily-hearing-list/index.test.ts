...
vi.mock("@hmcts/utiac-jr-birmingham-daily-hearing-list", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@hmcts/utiac-jr-birmingham-daily-hearing-list")>();
  return {
    ...actual,
...
import { renderUtiacJrBirminghamDailyHearingListData } from "@hmcts/utiac-jr-birmingham-daily-hearing-list";
import { GET } from "./index.js";

describe("UTIAC JR Birmingham Daily Hearing List page controller", () => {
  ...
  describe("GET handler", () => {
    it("should render the list successfully with valid data", async () => {
      ...
      const mockJsonData = [
        {
          ...
          location: "Birmingham Civil Justice Centre",
      ...
      expect(renderUtiacJrBirminghamDailyHearingListData).toHaveBeenCalledWith(mockJsonData, {
        locale: "en",
    ...
    it("should pass displayFrom to renderer", async () => {
      ...
      const mockRenderedData = {
        header: { listTitle: "UTIAC JR Birmingham", listForDate: "15 January 2026", lastUpdatedDate: "14 January 2026", lastUpdatedTime: "12pm" },
        hearings: []
      };
      ...
      // Assert
      expect(renderUtiacJrBirminghamDailyHearingListData).toHaveBeenCalledWith([], expect.objectContaining({ displayFrom }));
    });
    ...
    it("should use Welsh locale when specified", async () => {
      ...
      // Assert
      expect(renderUtiacJrBirminghamDailyHearingListData).toHaveBeenCalledWith([], expect.objectContaining({ locale: "cy" }));
    });
  });
});
