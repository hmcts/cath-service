...
vi.mock("@hmcts/postgres-prisma", () => ({
  prisma: {
    listType: {
      findUnique: vi.fn().mockResolvedValue({ id: 9, allowedProvenance: "MANUAL_UPLOAD", isNonStrategic: true })
    }
  }
}));
...
vi.mock("@hmcts/care-standards-tribunal-weekly-hearing-list", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@hmcts/care-standards-tribunal-weekly-hearing-list")>();
  return {
    ...actual,
...
import { renderCareStandardsTribunalData } from "@hmcts/care-standards-tribunal-weekly-hearing-list";
