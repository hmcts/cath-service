...
vi.mock("@hmcts/utiac-jr-cardiff-daily-hearing-list", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@hmcts/utiac-jr-cardiff-daily-hearing-list")>();
  return {
    ...actual,
    renderUtiacJrCardiffDailyHearingListData: vi.fn()
  };
});

import { getArtefactById, getPublicationJson } from "@hmcts/publication";
import { renderUtiacJrCardiffDailyHearingListData } from "@hmcts/utiac-jr-cardiff-daily-hearing-list";
import { GET } from "./index.js";

describe("UTIAC JR Cardiff Daily Hearing List page controller", () => {
  ...
  describe("GET handler", () => {
    it("should render the list successfully with valid data", async () => {
      ...
      const mockJsonData = [
        {
          ...
          location: "Cardiff Civil Justice Centre",
      ...
      vi.mocked(renderUtiacJrCardiffDailyHearingListData).mockReturnValue(mockRenderedData);
      ...
      expect(renderUtiacJrCardiffDailyHearingListData).toHaveBeenCalledWith(mockJsonData, {
        locale: "en",
    ...
    it("should pass displayFrom to renderer", async () => {
      ...
      const mockRenderedData = {
        header: { listTitle: "UTIAC JR Cardiff", listForDate: "15 January 2026", lastUpdatedDate: "14 January 2026", lastUpdatedTime: "12pm" },
        hearings: []
      };
      ...
      // Assert
      expect(renderUtiacJrCardiffDailyHearingListData).toHaveBeenCalledWith([], expect.objectContaining({ displayFrom }));
    });
    ...
    it("should use Welsh locale when specified", async () => {
      ...
      // Assert
      expect(renderUtiacJrCardiffDailyHearingListData).toHaveBeenCalledWith([], expect.objectContaining({ locale: "cy" }));
    });
  });
});
