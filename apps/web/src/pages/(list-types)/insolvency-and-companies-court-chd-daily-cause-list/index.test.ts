...
vi.mock("@hmcts/insolvency-and-companies-court-chd-daily-cause-list", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@hmcts/insolvency-and-companies-court-chd-daily-cause-list")>();
  return {
    ...actual,
...
import { renderInsolvencyAndCompaniesCourtChdDailyCauseList } from "@hmcts/insolvency-and-companies-court-chd-daily-cause-list";
import { getArtefactById, getPublicationJson } from "@hmcts/publication";
import { GET } from "./index.js";

describe("Insolvency & Companies Court (ChD) Daily Cause List page controller", () => {
  ...
  describe("GET handler", () => {
    it("should render the list successfully with valid data", async () => {
      const mockArtefact = {
        ...
        locationId: "26",
        listTypeId: 999,
        listTypeName: "INSOLVENCY_AND_COMPANIES_COURT_CHD_DAILY_CAUSE_LIST",
      ...
      vi.mocked(renderInsolvencyAndCompaniesCourtChdDailyCauseList).mockReturnValue(mockRenderedData as any);
      ...
      expect(renderInsolvencyAndCompaniesCourtChdDailyCauseList).toHaveBeenCalledWith(mockJsonData, {
        locale: "en",
        contentDate: mockArtefact.contentDate,
        lastReceivedDate: mockArtefact.lastReceivedDate.toISOString()
      });
      const renderCall = vi.mocked(res.render!).mock.calls[0]!;
      expect(renderCall[0]).toBe("insolvency-and-companies-court-chd-daily-cause-list");
    ...
    it("should use Welsh locale when specified", async () => {
      ...
      const mockRenderedData = {
        header: {
          listTitle: "Rhestr Achosion Dyddiol Llys Cwmnïau ac achosion Ansolfedd (Adran Siawnsri)",
      ...
      expect(renderInsolvencyAndCompaniesCourtChdDailyCauseList).toHaveBeenCalledWith(
        mockJsonData,
        expect.objectContaining({
          locale: "cy"
        })
