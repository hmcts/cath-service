...
vi.mock("@hmcts/interim-applications-chd-daily-cause-list", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@hmcts/interim-applications-chd-daily-cause-list")>();
  return {
    ...actual,
...
import { renderInterimApplicationsChdDailyCauseList } from "@hmcts/interim-applications-chd-daily-cause-list";
...
const baseArtefact = {
  ...
  locationId: "26",
  listTypeId: 999,
  listTypeName: "INTERIM_APPLICATIONS_CHD_DAILY_CAUSE_LIST",
...
describe("Interim Applications List (ChD) Daily Cause List page controller", () => {
  ...
  it("should render the list successfully with valid data including the important-info block", async () => {
    ...
    vi.mocked(renderInterimApplicationsChdDailyCauseList).mockReturnValue(mockRenderedData as any);

    await GET(req as Request, res as Response);

    expect(mockValidate).toHaveBeenCalledWith(mockJsonData);
    expect(renderInterimApplicationsChdDailyCauseList).toHaveBeenCalledWith(mockJsonData, {
      locale: "en",
      contentDate: baseArtefact.contentDate,
      lastReceivedDate: baseArtefact.lastReceivedDate.toISOString()
    });
    const renderCall = vi.mocked(res.render!).mock.calls[0]!;
    expect(renderCall[0]).toBe("interim-applications-chd-daily-cause-list");
  ...
  it("should use the Welsh locale when specified", async () => {
    ...
    expect(renderInterimApplicationsChdDailyCauseList).toHaveBeenCalledWith(mockJsonData, expect.objectContaining({ locale: "cy" }));
  });
