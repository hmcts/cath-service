...
describe("PAAC Weekly Hearing List page controller", () => {
  ...
  describe("GET handler", () => {
    it("should render the list successfully with valid data", async () => {
      ...
      expect(renderSiacPoacPaacData).toHaveBeenCalledWith(MOCK_JSON_DATA, expect.objectContaining({ courtName: "Pathogens Access Appeal Commission" }));
      expect(res.render).toHaveBeenCalledWith("paac-weekly-hearing-list", expect.objectContaining({ dataSource: "Manual Upload" }));
    });
    ...
    it("should render in Welsh when locale is cy", async () => {
      ...
      expect(res.render).toHaveBeenCalledWith("paac-weekly-hearing-list", expect.objectContaining({ dataSource: "Lanlwytho â Llaw" }));
    });
  });
});
