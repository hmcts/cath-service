...
vi.mock("@hmcts/et-daily-list");

const mockArtefact = {
  artefactId: "test-id",
  locationId: "1",
  listTypeId: 999,
  listTypeName: "ET_DAILY_LIST",
...
describe("et-daily-list controller", () => {
  ...
  it("should return 404 when the JSON blob cannot be read", async () => {
    req.query = { artefactId: "test-id" };
    vi.mocked(getArtefactById).mockResolvedValue(mockArtefact);
  ...
  it("should return 400 when JSON validation fails", async () => {
    ...
    vi.mocked(validateEtDailyList).mockReturnValue({ isValid: false, errors: ["boom"] } as any);
    await GET(req as Request, res as Response);
    expect(res.status).toHaveBeenCalledWith(400);
  });
  ...
  it("should render the list with valid data in English", async () => {
    ...
    expect(renderCall[0]).toBe("et-daily-list");
