...
vi.mock("@hmcts/financial-list-chd-kb-daily-cause-list", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@hmcts/financial-list-chd-kb-daily-cause-list")>();
  return {
    ...actual,
    renderFinancialListChdKbDailyCauseList: vi.fn()
  };
});

import { renderFinancialListChdKbDailyCauseList } from "@hmcts/financial-list-chd-kb-daily-cause-list";
import { getArtefactById, getPublicationJson } from "@hmcts/publication";
import { GET } from "./index.js";

describe("Financial List (ChD/KB) Daily Cause List page controller", () => {
  ...
  describe("GET handler", () => {
    it("should render the list successfully with valid data", async () => {
      const mockArtefact = {
        ...
        locationId: "26",
        listTypeId: 999,
        listTypeName: "FINANCIAL_LIST_CHD_KB_DAILY_CAUSE_LIST",
      ...
      const mockRenderedData = {
        header: {
          listTitle: "Financial List (Chancery Division/King's Bench Division/Commercial Court) Daily Cause List",
      ...
      expect(renderFinancialListChdKbDailyCauseList).toHaveBeenCalledWith(mockJsonData, {
        locale: "en",
        contentDate: mockArtefact.contentDate,
        lastReceivedDate: mockArtefact.lastReceivedDate.toISOString()
      });
      const renderCall = vi.mocked(res.render!).mock.calls[0]!;
      expect(renderCall[0]).toBe("financial-list-chd-kb-daily-cause-list");
    ...
    it("should use Welsh locale when specified", async () => {
      ...
      const mockRenderedData = {
        header: {
          listTitle: "financial welsh title",
      ...
      expect(renderFinancialListChdKbDailyCauseList).toHaveBeenCalledWith(
        mockJsonData,
        expect.objectContaining({
          locale: "cy"
        })
