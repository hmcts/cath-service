...
import { financialListChdKbDailyCauseListCy as cy, financialListChdKbDailyCauseListEn as en } from "@hmcts/financial-list-chd-kb-daily-cause-list";
...
const TEMPLATE = "financial-list-chd-kb-daily-cause-list.njk";
...
describe("financial-list-chd-kb-daily-cause-list template", () => {
  ...
  describe("Important information section", () => {
    it("should render the details summary as 'Important information' with the Chancery Division and Commercial Court guidance inside", () => {
      const { $ } = renderPage();
    ...
    it("should include both justice.gov.uk contacts in the guidance", () => {
      const { $ } = renderPage();

      const detailsText = $(".govuk-details").text();
      expect(detailsText).toContain("chanceryjudgeslisting@justice.gov.uk");
      expect(detailsText).toContain("comct.listing@justice.gov.uk");
    });
  });
