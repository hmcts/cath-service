...
vi.mock("@hmcts/companies-winding-up-chd-daily-cause-list", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@hmcts/companies-winding-up-chd-daily-cause-list")>();
  return {
    ...actual,
...
import { renderCompaniesWindingUpChdDailyCauseList } from "@hmcts/companies-winding-up-chd-daily-cause-list";
  ...
  describe("GET handler", () => {
    it("should render the list successfully with valid data", async () => {
      const mockArtefact = {
        ...
        locationId: "26",
        listTypeId: 999,
        listTypeName: "COMPANIES_WINDING_UP_CHD_DAILY_CAUSE_LIST",
      ...
      vi.mocked(renderCompaniesWindingUpChdDailyCauseList).mockReturnValue(mockRenderedData as any);
      ...
      expect(renderCompaniesWindingUpChdDailyCauseList).toHaveBeenCalledWith(mockJsonData, {
        locale: "en",
        contentDate: mockArtefact.contentDate,
        lastReceivedDate: mockArtefact.lastReceivedDate.toISOString()
      });
      const renderCall = vi.mocked(res.render!).mock.calls[0]!;
      expect(renderCall[0]).toBe("companies-winding-up-chd-daily-cause-list");
    ...
    it("should use Welsh locale when specified", async () => {
      ...
      const mockRenderedData = {
        header: {
          listTitle: "Rhestr Achosion Dyddiol Dirwyn Cwmnïau i Ben (Adran Siawnsri)",
      ...
      expect(renderCompaniesWindingUpChdDailyCauseList).toHaveBeenCalledWith(
        mockJsonData,
        expect.objectContaining({
          locale: "cy"
        })
