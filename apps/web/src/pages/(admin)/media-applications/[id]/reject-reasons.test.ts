  ...
  describe("GET handler", () => {
    it("should render reject-reasons page in English", async () => {
      ...
      expect(renderSpy).toHaveBeenCalledWith(
        "media-applications/[id]/reject-reasons",
        expect.objectContaining({
          pageTitle: "Select rejection reasons",
          selectAllText: "Select all that apply",
          checkboxLegend: "Why are you rejecting this application?",
          reasons: {
            notAccredited: "Not an accredited member of the media",
            invalidId: "Invalid proof of ID",
            detailsMismatch: "Details on application do not match proof of ID"
          },
    ...
    it("should render in Welsh when lng=cy", async () => {
      ...
      expect(renderSpy).toHaveBeenCalledWith(
        "media-applications/[id]/reject-reasons",
        expect.objectContaining({
          pageTitle: "Dewiswch resymau dros wrthod",
          selectAllText: "Dewiswch bob un sy'n berthnasol",
          checkboxLegend: "Pam rydych chi'n gwrthod y cais hwn?",
          reasons: {
            notAccredited: "Nid yw'n aelod achrededig o'r cyfryngau",
            invalidId: "Prawf o ID annilys",
            detailsMismatch: "Nid yw manylion ar y cais yn cyd-fynd â'r prawf o ID"
          },
  ...
  describe("POST handler", () => {
    it("should show validation error when no reasons selected", async () => {
      ...
      expect(renderSpy).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          id: "app-123",
          selectedReasons: {},
          errors: [{ text: "Select at least one reason for rejection", href: "#notAccredited" }],
          hideLanguageToggle: true
        })
      );
    });
    ...
    it("should show validation error in Welsh when lng=cy and no reasons selected", async () => {
      ...
      expect(renderSpy).toHaveBeenCalledWith("media-applications/[id]/reject-reasons", {
        ...
        id: "app-123",
        selectedReasons: {},
        errors: [{ text: "Dewiswch o leiaf un rheswm dros wrthod", href: "#notAccredited" }],
        hideLanguageToggle: true
      });
    });
