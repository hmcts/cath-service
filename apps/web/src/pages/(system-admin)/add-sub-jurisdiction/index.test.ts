...
describe("add-sub-jurisdiction page", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(systemAdminPages.getAllJurisdictions).mockResolvedValue([
  ...
  describe("GET", () => {
    it("should render the add-sub-jurisdiction page with English content", async () => {
      ...
      expect(systemAdminPages.getAllJurisdictions).toHaveBeenCalled();
  ...
  describe("POST", () => {
    it("should create sub-jurisdiction and redirect to success page on valid data", async () => {
      ...
      vi.mocked(systemAdminPages.validateSubJurisdictionData).mockResolvedValue([]);

      await POST(mockRequest as Request, mockResponse as Response);

      expect(systemAdminPages.createSubJurisdiction).toHaveBeenCalledWith(1, "Civil Court", "Llys Sifil");
    ...
    it("should render form with errors on validation failure", async () => {
      ...
      vi.mocked(systemAdminPages.validateSubJurisdictionData).mockResolvedValue(errors);
      ...
      expect(systemAdminPages.createSubJurisdiction).not.toHaveBeenCalled();
    ...
    it("should reload jurisdiction dropdown on validation failure", async () => {
      ...
      expect(systemAdminPages.getAllJurisdictions).toHaveBeenCalledTimes(1);
      expect(mockResponse.render).toHaveBeenCalledWith(
        "add-sub-jurisdiction/index",
        expect.objectContaining({
