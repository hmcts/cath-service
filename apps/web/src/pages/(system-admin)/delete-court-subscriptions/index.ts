...
import { AuditLogAction, performLocationSubscriptionsDeletion, validateDeleteCourtRadioSelection as validateRadioSelection } from "@hmcts/system-admin-pages";
...
function renderConfirmationPage(
  ...
  res.render("delete-court-subscriptions/index", {
    ...content,
...
const postHandler = async (req: Request, res: Response) => {
  ...
  await performLocationSubscriptionsDeletion(session.deleteCourt.locationId, session.deleteCourt.name, req.user?.email ?? "unknown");

  req.auditMetadata = {
    shouldLog: true,
    action: AuditLogAction.DELETE_COURT_SUBSCRIPTIONS,
    entityInfo: `Name: ${session.deleteCourt.name}, Location ID: ${session.deleteCourt.locationId}`
  };

  res.redirect(buildRedirectUrl("/delete-court-subscriptions-success", language));
};
