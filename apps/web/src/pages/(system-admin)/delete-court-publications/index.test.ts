...
vi.mock("@hmcts/system-admin-pages", async (importOriginal) => {
  ...
  return {
    ...
    performLocationPublicationsDeletion: vi.fn(),
    validateDeleteCourtRadioSelection: vi.fn()
  };
});

import { getLocationWithDetails } from "@hmcts/location";
import { performLocationPublicationsDeletion, validateDeleteCourtRadioSelection } from "@hmcts/system-admin-pages";
...
describe("delete-court-publications page", () => {
  ...
  describe("getHandler", () => {
    ...
    it("should render the confirmation page with location details", async () => {
      ...
      expect(res.render).toHaveBeenCalledWith(
        "delete-court-publications/index",
  ...
  describe("postHandler", () => {
    it("should redirect to delete-court when user selects no", async () => {
      ...
      expect(res.redirect).toHaveBeenCalledWith("/delete-court");
      expect(performLocationPublicationsDeletion).not.toHaveBeenCalled();
    });

    it("should delete publications and redirect to the success page when user selects yes", async () => {
      req.session = { deleteCourt: { locationId: 123, name: "Test", welshName: "Prawf" } } as any;
      req.body = { confirmDelete: "yes" };
      req.user = { email: "requester@example.com" } as any;
      (validateDeleteCourtRadioSelection as any).mockReturnValue(null);
      (performLocationPublicationsDeletion as any).mockResolvedValue(undefined);

      await postHandler(req as Request, res as Response);

      expect(performLocationPublicationsDeletion).toHaveBeenCalledWith(123, "Test", "requester@example.com");
      expect(req.auditMetadata).toEqual(expect.objectContaining({ shouldLog: true, action: "Delete court publications" }));
      expect(res.redirect).toHaveBeenCalledWith("/delete-court-publications-success");
    });
