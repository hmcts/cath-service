...
import { searchUsers } from "@hmcts/system-admin-pages";
...
describe("find-users page", () => {
  ...
  describe("GET handler", () => {
    ...
    it("should render find-users page with session filters", async () => {
      ...
      // Assert
      expect(searchUsers).toHaveBeenCalledWith(mockFilters, 1);
      expect(mockResponse.render).toHaveBeenCalledWith(
        "find-users/index",
        expect.objectContaining({
    ...
    it("should handle database errors gracefully", async () => {
      // Arrange
      vi.mocked(searchUsers).mockRejectedValue(new Error("Database connection failed"));
    ...
    it("should use page from query string", async () => {
      ...
      // Assert
      expect(searchUsers).toHaveBeenCalledWith({}, 3);
    });
