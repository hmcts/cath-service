...
const postHandler = async (req: Request, res: Response) => {
  ...
  if (action === "remove-location" && req.body.locationId) {
    ...
    const updatedLocations = currentLocations.filter((id: string) => id !== req.body.locationId);
  ...
  if (action === "remove-list-type" && listTypeId) {
    ...
    req.session.emailSubscriptions.pendingListTypeIds = currentIds.filter((id: number) => id !== idToRemove);
  ...
  if (action === "confirm") {
    ...
    const resolvedLocationIds = (
      await Promise.all(
        confirmedLocations.map(async (id: string) => {
          const location = await getLocationById(Number.parseInt(id, 10));
    ...
    const allowedListTypeIds = await getAllowedListTypeIdsForLocations(resolvedLocationIds.map((id: string) => Number.parseInt(id, 10)));
    const validListTypeIds = pendingListTypeIds.filter((id: number) => allowedListTypeIds.includes(id));
