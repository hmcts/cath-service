import { getB2cBaseUrl, getB2cConfig, isB2cConfigured } from "@hmcts/auth";
...
describe("B2C Login Page", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(isB2cConfigured).mockReturnValue(true);
    ...
    vi.mocked(getB2cBaseUrl).mockReturnValue("https://sign-in.test.platform.hmcts.net/test.platform.hmcts.net");
  });
  ...
  it("should redirect to B2C authorization URL", () => {
    ...
    expect(redirectUrl).toContain("B2C_1_SignInUserFlow");
  ...
  it("should store locale in session for callback redirect", () => {
    req.query = { lng: "cy" };
  ...
  it("should store provider in session", () => {
    GET(req as Request, res as Response);

    expect(req.session?.b2cProvider).toBe("cath");
  });

  it("should store default returnTo in session", () => {
    GET(req as Request, res as Response);

    expect(req.session?.returnTo).toBe("/account-home");
  });

  it("should preserve existing returnTo in session", () => {
    req.session = { id: "test", returnTo: "/admin" } as any;
  ...
  it("should return 503 if B2C is not configured", () => {
    vi.mocked(isB2cConfigured).mockReturnValue(false);
