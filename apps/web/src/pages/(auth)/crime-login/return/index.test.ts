import { exchangeCrimeCodeForToken, getCrimeIdamConfig } from "@hmcts/auth";
...
describe("crime-callback", () => {
  ...
  describe("GET", () => {
    ...
    it("should redirect to crime-rejected when user has rejected role", async () => {
      // Arrange
      const { exchangeCrimeCodeForToken: exchangeCodeForToken, extractCrimeUserInfoFromToken: extractUserInfoFromToken } = await import("@hmcts/auth");
      const { getCrimeIdamConfig } = await import("@hmcts/auth");
      const { isRejectedCrimeRole } = await import("@hmcts/auth");
    ...
    it("should redirect to sign-in with auth_failed when token exchange fails", async () => {
      ...
      vi.mocked(exchangeCrimeCodeForToken).mockRejectedValue(new Error("Token exchange failed"));
