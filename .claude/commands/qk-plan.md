...
2. **Implementation Details**
   ...
   - For any new rendered view/page, an equivalent Nunjucks template usually already exists in pip-frontend. Ask the user whether the template should be migrated from there; if so, note in the plan that implementation follows the `migrate-pip-pages` skill for the `.njk` and its en/cy content. Just record the decision and the source page — do NOT copy the skill's fetch/adapt/verify steps into the plan; implement runs the skill itself. (It does not apply to flat-file / manual-upload list types or registration-only work.)
