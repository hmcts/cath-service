...
```
apps/
├── web/                       # Frontend application (composition only)
│   └── src/
│       └── views/             # Shared layouts and error templates
└── api/                       # Backend API (composition only)
    └── src/
...
└── [feature]/
    ...
    └── src/
        ...
        ├── pages/             # Page routes - each page is a directory
        │   └── my-page/       # Page directory
...
// Core exports - include what your module needs
export const pageRoutes = { path: path.join(__dirname, "pages") };
export const assets = path.join(__dirname, "assets/");
export const moduleRoot = __dirname;  // Commonly needed for path resolution
...
// Export as RequestHandler[] array with middleware chain
export const GET: RequestHandler[] = [
  requireRole([USER_ROLES.SYSTEM_ADMIN, USER_ROLES.INTERNAL_ADMIN_CTSC]),
  getHandler
];
```
...
```typescript
// libs/user-management/src/user/service.ts
import { findUserById, createUserInDb } from "./queries.js";
...
- Extend `layouts/base-template.njk` (singular, not `base-templates` or `default`)
- Use `{% block page_content %}` (not `content`)
- Pass flat props from controller, not nested `en`/`cy` objects
- Include `pageTitle` for the heading
...
- [ ] Module structure follows convention (pages/[page-name]/ with index.ts, index.njk, en.ts, cy.ts)
