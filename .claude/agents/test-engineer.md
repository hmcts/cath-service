...
**Testing RequestHandler[] page controllers:**
```typescript
// Controllers export GET/POST as RequestHandler[] arrays
// The actual handler is the last element in the array

import { GET, POST } from "./index.js";

// Extract the handler (last element) from the middleware chain
const getHandler = GET[GET.length - 1];
const postHandler = POST[POST.length - 1];

// Test with mocked request/response
const mockReq = {
  query: { lng: 'en' },
  body: { field: 'value' },
...
expect(mockRes.render).toHaveBeenCalledWith('page-name/index', expect.objectContaining({
  pageTitle: expect.any(String)
}));
```

**Testing public pages (simple exports):**
```typescript
// Public pages may export GET/POST directly as functions
import { GET, POST } from "./index.js";
