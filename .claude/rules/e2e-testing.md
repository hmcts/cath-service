...
test.describe("Feature Name", () => {
  test("user can complete journey @nightly", async ({ page }) => {
    ...
    // 2. Test Welsh
    await page.getByRole("link", { name: "Cymraeg" }).click();
    await expect(page.getByRole("heading", { level: 1 })).toContainText("Dechrau nawr");
...
Use AxeBuilder with WCAG tags for comprehensive accessibility checks:
...
If specific rules need to be disabled due to known issues:

```typescript
const results = await new AxeBuilder({ page })
  .withTags(["wcag2a", "wcag2aa", "wcag21a", "wcag21aa", "wcag22aa"])
  .disableRules(["target-size", "link-name"]) // Document why in a comment
  .analyze();
```
