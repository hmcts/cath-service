...
Controllers can export arrays of middleware for protected routes:

```typescript
import type { RequestHandler } from "express";
import { requireRole } from "@hmcts/auth";
import { USER_ROLES } from "../constants.js";

const getHandler = async (req: Request, res: Response) => {
  res.render("admin-page", { en, cy, t });
};
...
<div class="govuk-grid-row">
  <div class="govuk-grid-column-two-thirds">
    ...
    <form method="post" novalidate>
      ...
      {{ govukButton({ text: t.continueButtonText }) }}
    </form>
...
- **Extend `layouts/base-template.njk`** - the standard layout for this project
- **Import macros at top** - use `{% from %}` for GOV.UK components
- **Use `{{ t.key }}` for translated content** - passed from controller
...
|--------------|----------|
| Page titles, headings, body text | Page-level `en.ts`/`cy.ts` files |
...
{{ govukInput({
  id: "name",
  name: "name",
  label: { text: t.nameLabel, isPageHeading: true, classes: "govuk-label--l" },
  value: data.name,
...
{{ govukRadios({
  ...
  fieldset: { legend: { text: t.contactPreferenceLabel, isPageHeading: true, classes: "govuk-fieldset__legend--l" } },
  items: [
    { value: "email", text: t.emailOptionText },
    { value: "phone", text: t.phoneOptionText }
  ],
...
{{ govukCheckboxes({
  name: "notifications",
  fieldset: { legend: { text: t.notificationsLabel } },
...
{{ govukDateInput({
  ...
  fieldset: { legend: { text: t.dateOfBirthLabel, isPageHeading: true, classes: "govuk-fieldset__legend--l" } },
  hint: { text: t.dateHintText },
  items: [
...
- ❌ Extending wrong layout (use `layouts/base-template.njk`)
