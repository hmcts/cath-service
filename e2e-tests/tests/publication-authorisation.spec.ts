...
test.describe("Publication Authorisation - Summary of Publications", () => {
  ...
  test.describe("Comprehensive Accessibility and Keyboard Navigation Journey @nightly", () => {
    test("should maintain WCAG 2.2 AA compliance and keyboard operability throughout complete authenticated user journey", async ({ page }) => {
      // 1. Authenticate as CFT user
      await page.goto("/sign-in");
      ...
      // 2. Test accessibility on authenticated summary page
      await page.goto("/summary-of-publications?locationId=9");
      await page.waitForSelector("h1.govuk-heading-l");
      ...
      // 3. Test comprehensive keyboard navigation - Tab through all interactive elements
      const heading = page.locator("h1.govuk-heading-l");
      await heading.focus();
      ...
      // Tab through all elements and record interactive ones
      while (tabCount < maxTabs) {
        await page.keyboard.press("Tab");
        tabCount++;
        ...
        // Record unique interactive elements
        ...
        // Break if we've cycled back to the heading
        const currentHref = await page.evaluate(() => (document.activeElement as HTMLAnchorElement)?.href || "");
        if (tabCount > 10 && currentHref === "" && activeElement.tagName === "H1") {
          break;
        }
      }
      ...
      // Verify publication links are keyboard accessible
      const publicationLinkElements = interactiveElements.filter((el) => el.href?.includes("artefactId="));
      expect(publicationLinkElements.length).toBeGreaterThan(0);

      // 4. Test focus management - navigate to a publication and verify focus
      await page.goto("/summary-of-publications?locationId=9");
      await page.waitForSelector("h1.govuk-heading-l");
      ...
      // Test link activation (keyboard users can use Space or Enter)
      await focusedElement.click();
      await page.waitForLoadState("networkidle");

      const publicationUrl = page.url();
      expect(publicationUrl).toContain("artefactId=");

      // 5. Test accessibility on publication detail page
      ...
      // 6. Test keyboard navigation on detail page - Back link should be accessible
      await page.keyboard.press("Tab");
      ...
      for (let i = 0; i < 20; i++) {
        ...
        if (focusedText.toLowerCase().includes("back") || focusedHref.includes("summary-of-publications")) {
      ...
      // 7. Navigate back to summary and test Welsh accessibility
      await page.goto("/summary-of-publications?locationId=9&lng=cy");
      await page.waitForSelector("h1.govuk-heading-l");
      ...
      // 8. Test keyboard navigation to find language-related links
      await page.goto("/summary-of-publications?locationId=9");
      await page.waitForSelector("h1.govuk-heading-l");

      let languageLinkFocused = false;
      await page.keyboard.press("Tab");
      ...
      for (let i = 0; i < 30; i++) {
        ...
        // Check if we found a language-related link (Cymraeg or lng=cy parameter)
        if (focusedText === "Cymraeg" || focusedHref.includes("lng=cy") || focusedHref.includes("cymraeg")) {
          languageLinkFocused = true;
          // Verify it's keyboard accessible (we found it via Tab)
          const linkElement = page.locator(":focus");
      ...
      // Verify we found a language link via keyboard
      expect(languageLinkFocused).toBe(true);

      // 9. Test skip link accessibility (WCAG requirement)
      await page.goto("/summary-of-publications?locationId=9");
      await page.reload(); // Ensure fresh page load
      ...
      // Verify first tab lands on an interactive element
      expect(firstFocusedElement.tagName).toMatch(/A|BUTTON|INPUT/);

      // 10. Final comprehensive accessibility check after logout
      await logout(page);
      await page.goto("/summary-of-publications?locationId=9");
      await page.waitForSelector("h1.govuk-heading-l");
      ...
      // Verify keyboard navigation still works after logout
      await page.keyboard.press("Tab");
      const postLogoutFocused = await page.evaluate(() => document.activeElement?.tagName);
      expect(postLogoutFocused).toBeTruthy();
    });
  });
});
