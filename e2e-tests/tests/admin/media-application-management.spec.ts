...
import { createTestMediaApplication, deleteTestMediaApplication, getTestMediaApplication } from "../../utils/test-support-api.js";
...
test.describe("Media Application Management", () => {
  ...
  test.afterAll(async () => {
    if (approvalApplicationId) {
      await deleteTestMediaApplication(approvalApplicationId).catch(() => {
        // Ignore if already deleted
      });
    }

    if (rejectionApplicationId) {
      await deleteTestMediaApplication(rejectionApplicationId).catch(() => {
        // Ignore if already deleted
      });
    }
  });
  ...
  test.describe("CTSC Admin Workflows", () => {
    ...
    test("should complete media application approval journey", async ({ page }) => {
      ...
      const testApp = (await createTestMediaApplication({
      ...
      await deleteTestMediaApplication(testApp.id);
    });
