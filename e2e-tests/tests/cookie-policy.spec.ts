...
test.describe("Cookie Policy Page", () => {
  test("should complete full cookie policy user journey", async ({ page }) => {
    // Start from home page and verify footer link
    await page.goto("/");
    await page.waitForLoadState("load");

    // Check footer link exists and opens in the current tab
    ...
    // Click footer link and verify it navigates in the current tab
    await footerLink.click();
    await page.waitForLoadState("load");
    await expect(page).toHaveURL(/\/cookie-policy/);

    // Continue journey on the same page
    await expect(page.locator("h1")).toHaveText("Cookie policy");

    // Verify back button is not displayed (cookie policy page suppresses the back link)
    ...
    await page.waitForURL(/\/cookie-policy\?saved=true/, { timeout: 10000 });
    await page.waitForLoadState("load");
