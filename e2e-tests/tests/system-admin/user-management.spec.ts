...
import { generateTestEmail } from "../../utils/test-prefix.js";
import { createTestUser } from "../../utils/test-support-api.js";
...
// Helper to create a test user via the API with proper prefix for cleanup
async function createPrefixedTestUser(localPart: string, role: string, provenance: string): Promise<{ userId: string; email: string }> {
  const email = generateTestEmail(`${localPart}.${randomUUID().slice(0, 8)}`);
  const result = await createTestUser({
    ...
    userProvenanceId: `test-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`
  });
...
test.describe("User Management", () => {
  test("system admin can search, filter, view, and delete users @nightly", async ({ page }) => {
    // Create test users for searching (with prefix for automatic cleanup)
    const { userId: testUserId, email: testEmail } = await createPrefixedTestUser("test.user", "VERIFIED", "CFT_IDAM");
    await createPrefixedTestUser("another.user", "INTERNAL_ADMIN_CTSC", "SSO");
    ...
    await page.getByLabel("Email").fill(testEmail.substring(0, 20));
    ...
    // Verify user was deleted by searching for them and confirming no results
  ...
  test("system admin can cancel user deletion @nightly", async ({ page }) => {
    // Create test user (with prefix for automatic cleanup)
    const { email: testEmail } = await createPrefixedTestUser("cancel.test", "VERIFIED", "B2C");
    ...
    // Verify user still exists by searching for them
    await page.goto("/find-users");
    await page.getByLabel("Email").fill(testEmail);
    await page.getByRole("button", { name: "Apply filters" }).click();
  ...
  test("system admin can filter by user ID @nightly", async ({ page }) => {
    // Create test user (with prefix for automatic cleanup)
    const { userId: testUserId, email: testEmail } = await createPrefixedTestUser("userid.test", "SYSTEM_ADMIN", "CRIME_IDAM");
