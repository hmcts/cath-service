...
// Helper to create a test court for deletion (uses test prefix for automatic cleanup)
async function createTestCourt(name: string, welshName: string): Promise<number> {
  const result = await createTestLocation({
    name: prefixName(name),
    welshName: prefixName(welshName),
    regionIds: [4],
...
test.describe
  .skip("Delete Court Journey", () => {
    test.beforeEach(async ({ page }) => {
      await authenticateSystemAdmin(page);
    });

    // Note: No afterAll cleanup needed - all data uses test prefix and is cleaned up by global teardown
    ...
    test("user can complete delete court journey with validation checks", async ({ page }) => {
      ...
      const successAccessibility = await axeCheck(page).analyze();
    ...
    test("should block deletion of court with active subscriptions", async ({ page }) => {
      ...
      // Create a test user for the subscription (uses test prefix via generateTestEmail)
      const testUser = await createTestUser({
        email: generateTestEmail(`delete-court-${Date.now()}`),
        firstName: "Test",
        surname: "User",
      ...
      await createTestArtefact({
        ...
        displayTo: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
        isFlatFile: false,
