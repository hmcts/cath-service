...
async function createTestPublication(): Promise<TestPublicationData> {
  ...
  const location = (await getFirstTestLocation()) as { locationId: number; name: string };
  if (!location) {
    throw new Error("No location found in database");
  }

  // Create test artefact (JSON publication)
  const artefact = await createTestArtefact({
    locationId: location.locationId.toString(),
    provenance: "MANUAL_UPLOAD",
    displayFrom: new Date("2024-01-01").toISOString(),
    displayTo: new Date("2024-12-31").toISOString(),
    language: "ENGLISH",
    listTypeId: 1,
    sensitivity: "PUBLIC",
    contentDate: new Date("2024-06-01").toISOString(),
    isFlatFile: false
  });
  ...
  await createTestSubscription({
    ...
    searchValue: location.locationId.toString()
  });
...
async function deleteTestPublication(publicationData: TestPublicationData): Promise<void> {
  try {
    ...
    try {
      await deleteTestSubscriptions({ userId: publicationData.userId });
    } catch (error) {
    ...
    // Delete test user
    try {
      await deleteTestUser(publicationData.userId);
    } catch (error) {
      console.log("User cleanup error:", error);
    }

    // Delete artefact
    await deleteTestArtefact(publicationData.artefactId);
