...
test.describe("Create Media Account", () => {
  ...
  test("should display form with all required fields", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { level: 1 })).toContainText("Create a Court and tribunal hearings account");
  ...
  test("should show error for invalid email @nightly", async ({ page }) => {
    ...
    await expect(page.locator(".govuk-error-summary")).toBeVisible();
    await expect(page.locator("#email-error")).toContainText("Enter an email address in the correct format, like name@example.com");
  });
  ...
  test("should show error for invalid file type @nightly", async ({ page }) => {
    ...
    await expect(page.locator(".govuk-error-summary")).toBeVisible();
    await expect(page.locator("#idProof-error")).toContainText("ID evidence must be a JPG, PDF or PNG");
  });
  ...
  test("should show error for file larger than 2MB @nightly", async ({ page }) => {
    ...
    await expect(page.locator(".govuk-error-summary")).toBeVisible();
    await expect(page.locator("#idProof-error")).toContainText("ID evidence needs to be less than 2Mbs");
  });
  ...
  test("should complete successful submission", async ({ page }) => {
    ...
    await expect(page.locator(".govuk-panel__title")).toContainText("Details submitted");
    await expect(page.getByRole("heading", { name: "What happens next" })).toBeVisible();
    await expect(page.locator("main .govuk-body").first()).toContainText("HMCTS will review your details");
  });
  ...
  test("should verify database record created after successful submission @nightly", async ({ page }) => {
    ...
    if (application) {
      const filePath = path.join(process.cwd(), "../apps/web/storage", "temp", "files", `${application.id}.pdf`);
  ...
  test("should pass accessibility checks on confirmation page", async ({ page }) => {
    await page.goto("/create-media-account");
