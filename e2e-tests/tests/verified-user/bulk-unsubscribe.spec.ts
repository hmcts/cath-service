...
async function createTestDataForBulkUnsubscribe(): Promise<TestData> {
...
async function deleteTestDataForBulkUnsubscribe(testData: TestData): Promise<void> {
  try {
    await deleteTestSubscriptions({
      searchType: "LOCATION_ID",
      searchValues: [testData.locationId1.toString(), testData.locationId2.toString(), testData.locationId3.toString()]
    });

    await deleteTestLocations([testData.locationId1, testData.locationId2, testData.locationId3]);
  } catch (error) {
...
test.describe("Bulk Unsubscribe", () => {
  test.beforeEach(async ({ page }, testInfo) => {
    const testData = await createTestDataForBulkUnsubscribe();
  ...
  test.afterEach(async ({}, testInfo) => {
    ...
    if (testData) {
      await deleteTestDataForBulkUnsubscribe(testData);
  ...
  test("Verified user can bulk unsubscribe", async ({ page }, testInfo) => {
    ...
    const remainingSubscriptions1 = (await getTestSubscriptions({
      searchType: "LOCATION_ID",
      searchValue: testData.locationId1.toString()
    })) as unknown[];

    const remainingSubscriptions2 = (await getTestSubscriptions({
      searchType: "LOCATION_ID",
      searchValue: testData.locationId2.toString()
    })) as unknown[];
