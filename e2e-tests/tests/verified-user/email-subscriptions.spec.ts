...
import { createTestLocation, deleteTestLocation, deleteTestSubscriptions, getFirstRegion, getFirstSubJurisdiction } from "../../utils/test-support-api.js";
...
async function createTestLocationForSubscription(): Promise<TestLocationData> {
  ...
  // Get the first sub-jurisdiction and region to link to via API
  const subJurisdiction = (await getFirstSubJurisdiction()) as { subJurisdictionId: number } | null;
  const region = (await getFirstRegion()) as { regionId: number } | null;
...
async function deleteTestLocationForSubscription(locationData: TestLocationData): Promise<void> {
  try {
    if (!locationData.locationId) return;

    // Delete subscriptions first via API (if any)
    await deleteTestSubscriptions({ searchType: "LOCATION_ID", searchValues: [String(locationData.locationId)] });

    // Delete location via API (cascade will handle relationships)
    await deleteTestLocation(locationData.locationId);
  } catch (error) {
...
test.describe("Email Subscriptions", () => {
  ...
  test.beforeEach(async ({ page }, testInfo) => {
    ...
    const locationData = await createTestLocationForSubscription();
  ...
  test.afterEach(async ({}, testInfo) => {
    ...
    if (locationData) {
      await deleteTestLocationForSubscription(locationData);
