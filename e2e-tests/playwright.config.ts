...
export default defineConfig({
  ...
  webServer: {
    ...
    command: process.env.CI
      ? `NODE_ENV=development ENABLE_SSO=true ENABLE_CFT_IDAM=true ENABLE_CRIME_IDAM=true ENABLE_B2C=true CRIME_IDAM_BASE_URL="${process.env.CRIME_IDAM_BASE_URL || "https://login.sit.cjscp.org.uk"}" CRIME_IDAM_CLIENT_ID="${process.env.CRIME_IDAM_CLIENT_ID || "test-client-id"}" CRIME_IDAM_CLIENT_SECRET="${process.env.CRIME_IDAM_CLIENT_SECRET || "test-secret"}" GOVUK_NOTIFY_API_KEY="${process.env.GOVUK_NOTIFY_API_KEY || ""}" GOVUK_NOTIFY_TEMPLATE_ID_SUBSCRIPTION="${process.env.GOVUK_NOTIFY_TEMPLATE_ID_SUBSCRIPTION || ""}" yarn dev:ci`
      : `NODE_ENV=development ENABLE_SSO=true ENABLE_CFT_IDAM=true ENABLE_CRIME_IDAM=true ENABLE_B2C=true CRIME_IDAM_BASE_URL="${process.env.CRIME_IDAM_BASE_URL || "https://login.sit.cjscp.org.uk"}" CRIME_IDAM_CLIENT_ID="${process.env.CRIME_IDAM_CLIENT_ID || "test-client-id"}" CRIME_IDAM_CLIENT_SECRET="${process.env.CRIME_IDAM_CLIENT_SECRET || "test-secret"}" GOVUK_NOTIFY_API_KEY="${process.env.GOVUK_NOTIFY_API_KEY || ""}" GOVUK_NOTIFY_TEMPLATE_ID_SUBSCRIPTION="${process.env.GOVUK_NOTIFY_TEMPLATE_ID_SUBSCRIPTION || ""}" yarn dev:nowatch`,
