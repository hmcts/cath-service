...
export async function getApiAuthToken(): Promise<string> {
  ...
  const clientSecret = process.env.CATH_SERVICE_API_CLIENT_SECRET;

  if (!tenantId || !clientId || !clientSecret) {
    throw new Error(
      "Missing Azure AD credentials. Ensure AZURE_TENANT_ID, CATH_SERVICE_API_CLIENT_ID and CATH_SERVICE_API_CLIENT_SECRET are set. " +
