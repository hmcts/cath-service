...
import { generateFamilyDailyCauseListExcel } from "./excel-generator.js";
...
describe("generateFamilyDailyCauseListExcel", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("should combine each party with its legal advisor in a single column and not emit separate advisor columns", async () => {
    ...
    // Act
    const result = await generateFamilyDailyCauseListExcel(options);
    ...
    expect(header).not.toContain("Applicant Legal Advisor");
  ...
  it("should name the worksheet with a short name within Excel's 31-char limit", async () => {
    ...
    // Act
    await generateFamilyDailyCauseListExcel(options);

    // Assert
    const name = await readWorksheetName();
    expect(name).toBe("Family Daily Cause List");
