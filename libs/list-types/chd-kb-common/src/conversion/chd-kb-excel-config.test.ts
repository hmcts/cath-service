...
import { CHD_KB_EXCEL_CONFIG } from "./chd-kb-excel-config.js";
...
describe("CHD_KB_EXCEL_CONFIG", () => {
  it("should have the correct field structure", () => {
    expect(CHD_KB_EXCEL_CONFIG.fields).toHaveLength(7);
  ...
  it("should require at least 1 data row", () => {
    expect(CHD_KB_EXCEL_CONFIG.minRows).toBe(1);
  });
  ...
  describe("missing column validation", () => {
    it("should reject a sheet missing a required column", async () => {
      ...
      await expect(convertExcelToJson(buffer, CHD_KB_EXCEL_CONFIG)).rejects.toThrow(/Excel file must contain columns/);
    });
  });
  ...
  describe("time validation", () => {
    ...
    it("should reject a time without an am/pm suffix", async () => {
      ...
      const buffer = await createExcelBuffer(data);
      await expect(convertExcelToJson(buffer, CHD_KB_EXCEL_CONFIG)).rejects.toThrow(/Invalid time format/);
    });
