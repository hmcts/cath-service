...
interface PdfGenerationOptions extends BasePdfGenerationOptions<UtlcHearingList> {
  contentDate: Date;
}

export async function generateUtlcDailyHearingListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  try {
    const renderedData = renderUtlcDailyHearingListData(options.jsonData, {
