...
import type { MagistratesPublicListData, RenderOptions } from "../rendering/renderer.js";
import { renderMagistratesPublicListData } from "../rendering/renderer.js";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

interface PdfGenerationOptions extends BasePdfGenerationOptions<MagistratesPublicListData> {
  contentDate: Date;
}
...
export async function generateMagistratesPublicListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  try {
    ...
    const renderedData = await renderMagistratesPublicListData(options.jsonData, renderOptions);
