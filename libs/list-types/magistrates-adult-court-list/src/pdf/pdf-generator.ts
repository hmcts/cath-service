...
interface PdfGenerationOptions extends BasePdfGenerationOptions<MagistratesAdultCourtListData> {
  contentDate: Date;
}

export async function generateMagistratesAdultCourtListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  try {
    ...
    const renderedData = await renderMagistratesAdultCourtList(options.jsonData, renderOptions);
