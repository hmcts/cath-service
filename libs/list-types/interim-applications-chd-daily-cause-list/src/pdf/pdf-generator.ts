...
interface PdfGenerationOptions extends BasePdfGenerationOptions<InterimApplicationsChdData> {
  contentDate: Date;
}

export async function generateInterimApplicationsChdDailyCauseListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  try {
    const renderedData = renderInterimApplicationsChdDailyCauseList(options.jsonData, {
    ...
    const html = env.render("pdf-template.njk", {
      ...
      importantInfo: renderedData.importantInfo,
