...
interface PdfGenerationOptions extends BasePdfGenerationOptions<GrcWeeklyHearingList> {
  contentDate: Date;
}

export async function generateGrcWeeklyHearingListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  try {
    const renderedData = renderGrcWeeklyHearingListData(options.jsonData, {
