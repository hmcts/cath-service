...
import type { CrownDailyListData, RenderOptions } from "../models/types.js";
import { renderCrownDailyListData } from "../rendering/renderer.js";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

interface PdfGenerationOptions extends BasePdfGenerationOptions<CrownDailyListData> {
  contentDate: Date;
}

export async function generateCrownDailyListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  try {
    ...
    const renderedData = await renderCrownDailyListData(options.jsonData, renderOptions);
