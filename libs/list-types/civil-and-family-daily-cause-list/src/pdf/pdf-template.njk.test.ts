...
function baseData(locale: typeof en | typeof cy = en) {
  return {
    ...
    openJustice: {
      ...
      email: "cf.barnet.countycourt@justice.gov.uk",
      phone: "0300 123 5577"
    }
  };
}
...
describe("civil-and-family-daily-cause-list pdf-template", () => {
  describe("Important information", () => {
    it("should render the open justice contact sentence naming the supplied venue", () => {
      ...
      expect(infoBox.text()).toContain(en.openJusticeContact("Barnet Civil and Family Courts Centre", "cf.barnet.countycourt@justice.gov.uk", "0300 123 5577"));
    });

    it("should render the Welsh open justice contact sentence naming the supplied venue", () => {
      const { $ } = renderPdf([], { openJustice: { venueName: "Llys Prawf", email: "cf@example.com", phone: "0300 123 5577" } }, cy);

      const infoBox = $(".info-box");
      expect(infoBox.find("h3").text()).toContain(cy.importantInformation);
      expect(infoBox.text()).toContain(cy.openJusticeContact("Llys Prawf", "cf@example.com", "0300 123 5577"));
    });
  });
