...
import type { MagistratesPublicListData } from "@hmcts/magistrates-public-common";
import { renderMagistratesPublicList } from "@hmcts/magistrates-public-common";
import { generatePdfFromHtml } from "@hmcts/pdf-generation";
import { PROVENANCE_LABELS } from "@hmcts/publication";

export interface MagistratesPublicPdfOptions extends BasePdfGenerationOptions<MagistratesPublicListData> {
  contentDate: Date;
}

export async function generateMagistratesPublicAdultCourtListFuturePdf(options: MagistratesPublicPdfOptions): Promise<PdfGenerationResult> {
  const __filename = fileURLToPath(import.meta.url);
  const __dirname = path.dirname(__filename);

  try {
    const renderedData = await renderMagistratesPublicList(options.jsonData, {
