...
import { UTIAC_JR_CARDIFF_EXCEL_CONFIG } from "./utiac-jr-cardiff-config.js";

describe("UTIAC_JR_CARDIFF_EXCEL_CONFIG", () => {
  it("should have correct configuration structure", () => {
    expect(UTIAC_JR_CARDIFF_EXCEL_CONFIG).toBeDefined();
    expect(UTIAC_JR_CARDIFF_EXCEL_CONFIG.fields).toHaveLength(7);
    expect(UTIAC_JR_CARDIFF_EXCEL_CONFIG.minRows).toBe(1);
  });

  it("should have all fields with correct names", () => {
    const fieldNames = UTIAC_JR_CARDIFF_EXCEL_CONFIG.fields.map((f) => f.fieldName);
  ...
  it("should have correct required flags", () => {
    const requiredFields = UTIAC_JR_CARDIFF_EXCEL_CONFIG.fields.filter((f) => f.required).map((f) => f.fieldName);
    const optionalFields = UTIAC_JR_CARDIFF_EXCEL_CONFIG.fields.filter((f) => !f.required).map((f) => f.fieldName);
  ...
  describe("hearingTime field validation", () => {
    const hearingTimeField = UTIAC_JR_CARDIFF_EXCEL_CONFIG.fields.find((f) => f.fieldName === "hearingTime");
  ...
  describe("HTML tag validation on all text fields", () => {
    ...
    for (const fieldName of textFields) {
      it(`should reject HTML tags in ${fieldName}`, () => {
        const field = UTIAC_JR_CARDIFF_EXCEL_CONFIG.fields.find((f) => f.fieldName === fieldName);
