...
import type { CrownFirmListData, RenderOptions } from "../models/types.js";
import { renderCrownFirmListData } from "../rendering/renderer.js";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

interface PdfGenerationOptions extends BasePdfGenerationOptions<CrownFirmListData> {
  contentDate: Date;
}

export async function generateCrownFirmListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  try {
    ...
    const renderedData = await renderCrownFirmListData(options.jsonData, renderOptions);
