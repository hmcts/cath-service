...
interface PdfGenerationOptions extends BasePdfGenerationOptions<BusinessAndPropertyRollsData> {
  contentDate: Date;
}

export async function generateBusinessAndPropertyDivisionRollsBuildingDailyCauseListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  try {
    const renderedData = renderBusinessAndPropertyRolls(options.jsonData, {
    ...
    const html = env.render("pdf-template.njk", {
      ...
      sections: renderedData.sections,
