...
import { renderUtiacJrLondonDailyHearingListData } from "./renderer-london.js";

describe("renderUtiacJrLondonDailyHearingListData", () => {
  it("should render hearing list with formatted display date", () => {
    ...
    // Act
    const result = renderUtiacJrLondonDailyHearingListData(hearingList, options);

    // Assert
    expect(result.header.listTitle).toBe("Upper Tribunal (Immigration and Asylum) Chamber - Judicial Review: London Daily Hearing List");
