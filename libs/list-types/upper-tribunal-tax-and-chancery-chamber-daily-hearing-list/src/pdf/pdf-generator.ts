...
interface PdfGenerationOptions extends BasePdfGenerationOptions<UtccHearingList> {
  contentDate: Date;
}

export async function generateUtccDailyHearingListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  try {
    const renderedData = renderUtccDailyHearingListData(options.jsonData, {
