...
import { renderInterimApplicationsDailyCauseList } from "./renderer.js";

function buildData(overrides: Partial<InterimApplicationsData> = {}): InterimApplicationsData {
  return {
    hearingList: [
      {
        judge: "Judge A",
...
describe("renderInterimApplicationsDailyCauseList", () => {
  it("should render header and hearings from the hearingList", () => {
    const result = renderInterimApplicationsDailyCauseList(buildData(), OPTIONS);

    expect(result.header.listTitle).toBe("Interim Applications Daily Cause List");
  ...
  it("should reflect a different judge name and email per upload", () => {
    const result = renderInterimApplicationsDailyCauseList(
  ...
  it("should fall back to static wording when openJusticeStatementDetails is empty", () => {
    const result = renderInterimApplicationsDailyCauseList(buildData({ openJusticeStatementDetails: [] }), OPTIONS);
  ...
  it("should include the third paragraph about hearings not appearing in the individual list", () => {
    const result = renderInterimApplicationsDailyCauseList(buildData(), OPTIONS);

    expect(result.importantInfo.thirdParagraph).toBe(
  ...
  it("should use the Welsh list title and Welsh important-info wording when locale is cy", () => {
    const result = renderInterimApplicationsDailyCauseList(buildData({ openJusticeStatementDetails: [] }), { ...OPTIONS, locale: "cy" });

    expect(result.header.listTitle).toBe("Rhestr Achosion Dyddiol Ceisiadau Interim");
