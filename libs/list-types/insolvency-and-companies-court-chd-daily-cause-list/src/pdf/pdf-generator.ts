...
export async function generateInsolvencyAndCompaniesCourtChdDailyCauseListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  ...
  return generateListPdf<ChdKbHearingList>({
    ...
    renderData: (jsonData, { locale, contentDate, lastReceivedDate }) =>
      renderInsolvencyAndCompaniesCourtChdDailyCauseList(jsonData, { locale, contentDate, lastReceivedDate }),
