...
import { renderInsolvencyAndCompaniesCourtChdDailyCauseList } from "./renderer.js";

describe("renderInsolvencyAndCompaniesCourtChdDailyCauseList", () => {
  it("should render hearing list with header and hearings", () => {
    ...
    const result = renderInsolvencyAndCompaniesCourtChdDailyCauseList(hearingList, options);

    expect(result.header.listTitle).toBe("Insolvency & Companies Court (Chancery Division) Daily Cause List");
  ...
  it("should use the Welsh list title when locale is cy", () => {
    ...
    expect(result.header.listTitle).toBe("Rhestr Achosion Dyddiol Llys Cwmnïau ac achosion Ansolfedd (Adran Siawnsri)");
  });
