...
describe("renderCauseListData (civil-daily-cause-list)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
                        ...
                        hearing: [
                          {
                            ...
                            case: [{ caseName: "Smith v Jones", caseNumber: "CV-2025-001" }]
                          }
                        ]
                      }
                    ]
  ...
  it("should process applicant and respondent parties", async () => {
                            ...
                            case: [
                              {
                                ...
                                party: [
                                  ...
                                  },
                                  {
                                    ...
                                    individualDetails: { individualForenames: "Jane", individualSurname: "Jones" }
                                  }
                                ]
                              }
                            ]
    ...
    expect((caseItem as any).applicant).toBe("John Smith");
    expect((caseItem as any).respondent).toBe("Jane Jones");
  });
  ...
  it("should use Welsh location name when available", async () => {
    (getLocationById as any).mockResolvedValue({ name: "Oxford Court", welshName: "Llys Rhydychen" });

    const inputData = {
      document: { publicationDate: "2025-11-12T09:00:00.000Z" },
      venue: {
        venueName: "Oxford Court",
        venueAddress: { line: ["Address"], postCode: "OX1 1TL" }
      },
      courtLists: []
    };
    ...
    expect(result.header.locationName).toBe("Llys Rhydychen");
  });

  it("should format reporting restrictions", async () => {
    const inputData = {
      document: { publicationDate: "2025-11-12T09:00:00.000Z" },
      venue: {
        venueName: "Test Court",
