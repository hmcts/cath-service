...
import { renderCompetitionListChdDailyCauseList } from "./renderer.js";

describe("renderCompetitionListChdDailyCauseList", () => {
  it("should render hearing list with header and hearings", () => {
    ...
    const result = renderCompetitionListChdDailyCauseList(hearingList, options);

    expect(result.header.listTitle).toBe("Competition List (Chancery Division) Daily Cause List");
  ...
  it("should use the Welsh list title when locale is cy", () => {
    ...
    expect(result.header.listTitle).toBe("Rhestr Achosion Dyddiol Cystadleuaeth (Adran Siawnsri)");
  });
