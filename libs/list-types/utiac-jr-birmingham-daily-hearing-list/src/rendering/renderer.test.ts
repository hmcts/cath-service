...
import { renderUtiacJrBirminghamDailyHearingListData } from "./renderer.js";

describe("renderUtiacJrBirminghamDailyHearingListData", () => {
  it("should render hearing list with formatted display date", () => {
    ...
    // Act
    const result = renderUtiacJrBirminghamDailyHearingListData(hearingList, options);

    // Assert
    expect(result.header.listTitle).toBe("Upper Tribunal (Immigration and Asylum) Chamber - Judicial Review: Birmingham Daily Hearing List");
    ...
    expect(result.hearings[0].venue).toBe("Birmingham Civil Justice Centre");
