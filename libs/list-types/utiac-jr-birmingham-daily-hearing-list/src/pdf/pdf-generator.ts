...
interface PdfGenerationOptions extends BasePdfGenerationOptions<UtiacJrBirminghamHearingList> {
  displayFrom: Date;
}

export async function generateUtiacJrBirminghamDailyHearingListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  try {
    const renderedData = renderUtiacJrBirminghamDailyHearingListData(options.jsonData, {
