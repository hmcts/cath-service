...
import type { MagistratesListData, RenderOptions } from "../models/types.js";
import { renderMagistratesListData } from "../rendering/renderer.js";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

interface PdfGenerationOptions extends BasePdfGenerationOptions<MagistratesListData> {
  contentDate: Date;
}

export async function generateMagistratesStandardListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  try {
    ...
    const [renderedData, translations] = await Promise.all([
      renderMagistratesListData(options.jsonData, renderOptions),
      loadTranslations(
        options.locale,
        () => import("../pages/en.js"),
        () => import("../pages/cy.js")
