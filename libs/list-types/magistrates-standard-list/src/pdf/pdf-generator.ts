...
interface PdfGenerationOptions extends BasePdfGenerationOptions<MagistratesStandardList> {
  contentDate: Date;
}

export async function generateMagistratesStandardListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  try {
    const renderedData = await renderMagistratesStandardListData(options.jsonData, {
