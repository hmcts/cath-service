...
interface PdfGenerationOptions extends BasePdfGenerationOptions<WpafccWeeklyHearingList> {
  contentDate: Date;
}

export async function generateWpafccWeeklyHearingListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  try {
    const renderedData = renderWpafccWeeklyHearingListData(options.jsonData, {
