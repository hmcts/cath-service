...
interface PdfGenerationOptions extends BasePdfGenerationOptions<CourtOfAppealCivilData> {
  contentDate: Date;
}

export async function generateCourtOfAppealCivilDailyCauseListPdf(options: PdfGenerationOptions): Promise<PdfGenerationResult> {
  try {
    ...
    const html = env.render("pdf-template.njk", {
      ...
      dailyHearings: renderedData.dailyHearings,
      futureJudgments: renderedData.futureJudgments,
