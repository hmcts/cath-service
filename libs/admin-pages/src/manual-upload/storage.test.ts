...
import { getManualUpload, getNonStrategicUpload, storeManualUpload, storeNonStrategicUpload } from "./storage.js";
...
describe("non-strategic-upload-storage", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe("storeNonStrategicUpload", () => {
    it("should store upload data in Redis with 1 hour TTL", async () => {
      const testData = {
        file: Buffer.from("test file content"),
        fileName: "test.xlsx",
      ...
      const uploadId = await storeNonStrategicUpload(testData);

      expect(uploadId).toBe("test-uuid-123");
      expect(mockRedisClient.setEx).toHaveBeenCalledWith("non-strategic-upload:test-uuid-123", 3600, expect.any(String));

      const storedData = JSON.parse(mockRedisClient.setEx.mock.calls[0][2]);
      expect(storedData.fileName).toBe("test.xlsx");
      expect(storedData.fileType).toBe("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
      expect(storedData.fileBase64).toBe(Buffer.from("test file content").toString("base64"));
      expect(storedData.locationId).toBe("123");
      expect(storedData.listType).toBe("CIVIL_DAILY_CAUSE_LIST");
      expect(storedData.uploadedAt).toBeDefined();
    ...
    it("should store file as base64 encoded string", async () => {
      ...
      await storeNonStrategicUpload(testData);
  ...
  describe("getNonStrategicUpload", () => {
    it("should retrieve and decode upload data from Redis", async () => {
      const storedData = {
        fileName: "retrieved.xlsx",
        fileType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        fileBase64: Buffer.from("test content").toString("base64"),
        locationId: "999",
        listType: "CIVIL_DAILY_CAUSE_LIST",
      ...
      const result = await getNonStrategicUpload("test-uuid-123");

      expect(mockRedisClient.get).toHaveBeenCalledWith("non-strategic-upload:test-uuid-123");
      expect(result).toBeDefined();
      expect(result?.fileName).toBe("retrieved.xlsx");
    ...
    it("should return null for missing upload", async () => {
      ...
      const result = await getNonStrategicUpload("non-existent-id");

      expect(mockRedisClient.get).toHaveBeenCalledWith("non-strategic-upload:non-existent-id");
      expect(result).toBeNull();
    });
    ...
    it("should correctly decode base64 file data", async () => {
      ...
      const result = await getNonStrategicUpload("test-id");
