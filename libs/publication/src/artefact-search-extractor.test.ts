...
describe("extractAndStoreArtefactSearch", () => {
  ...
  it("should return the extracted cases", async () => {
    // Arrange
    const mockConfig = {
      caseNumberFieldName: "caseNumber",
      caseNameFieldName: "caseName"
    ...
    // Act
    const result = await extractAndStoreArtefactSearch("artefact-20", 1, mockPayload);
  ...
  it("should return an empty array when no list search config exists", async () => {
    // Arrange
    vi.mocked(getConfigForListType).mockResolvedValue(null as any);

    // Act
    const result = await extractAndStoreArtefactSearch("artefact-21", 1, { caseNumber: "CASE-123" });

    // Assert
    expect(result).toEqual([]);
  });

  it("should return an empty array when extraction throws", async () => {
    // Arrange
    vi.mocked(getConfigForListType).mockRejectedValue(new Error("config lookup failed"));

    // Act
    const result = await extractAndStoreArtefactSearch("artefact-22", 1, { caseNumber: "CASE-123" });
