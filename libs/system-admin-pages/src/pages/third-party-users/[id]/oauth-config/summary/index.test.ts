...
describe("third-party-users oauth-config summary page", () => {
  ...
  describe("getHandler", () => {
    it("should redirect to form when no session config", async () => {
      ...
      // Assert
      expect(res.redirect).toHaveBeenCalledWith("/third-party-users/user-1/oauth-config");
    });
    ...
    it("should render summary page with config from session", async () => {
      ...
      expect(res.render).toHaveBeenCalledWith(
        "third-party-users/[id]/oauth-config/summary/index",
  ...
  describe("postHandler", () => {
    ...
    it("should write secrets to Key Vault and redirect to success", async () => {
      ...
      expect(res.redirect).toHaveBeenCalledWith("/third-party-users/user-1/oauth-config/success");
    });
    ...
    it("should redirect to Welsh success page when language is cy", async () => {
      ...
      // Assert
      expect(res.redirect).toHaveBeenCalledWith("/third-party-users/user-1/oauth-config/success?lng=cy");
    });
  });
});
