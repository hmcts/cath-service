...
describe("PostgresStore", () => {
  ...
  describe("set", () => {
    it("should upsert session with TTL from cookie maxAge", async () => {
      const sessionData = { cookie: { maxAge: 7200000, originalMaxAge: 7200000 }, userId: "123" } as any;
      mockClient.query.mockResolvedValue({ rows: [] });
    ...
    it("should handle errors", async () => {
      ...
      await new Promise<void>((resolve) => {
        store.set("session123", { cookie: { maxAge: 3600000, originalMaxAge: 3600000 } } as any, (err) => {
          expect(err).toBe(error);
          resolve();
        });
      });
  ...
  describe("touch", () => {
    it("should update session expiration", async () => {
      const sessionData = { cookie: { maxAge: 3600000, originalMaxAge: 3600000 } } as any;
      mockClient.query.mockResolvedValue({ rows: [] });
    ...
    it("should skip if disableTouch is true", async () => {
      ...
      await new Promise<void>((resolve) => {
        touchStore.touch("session123", { cookie: { maxAge: 3600000, originalMaxAge: 3600000 } } as any, (err) => {
          expect(err).toBeNull();
