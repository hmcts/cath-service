...
const GOVUK_NOTIFY_TEMPLATE_ID_NON_SJP_PDF_EXCEL =
  process.env.GOVUK_NOTIFY_TEMPLATE_ID_NON_SJP_PDF_EXCEL || process.env.GOVUK_NOTIFY_TEMPLATE_ID_SUBSCRIPTION_PDF_EXCEL || "";
...
export function getSubscriptionTemplateId(params: { isSjp: boolean; hasPdf: boolean; hasExcel: boolean; filesUnder2MB: boolean }): string {
  ...
  if (hasPdf && hasExcel) {
    if (!GOVUK_NOTIFY_TEMPLATE_ID_NON_SJP_PDF_EXCEL) {
      throw new Error("GOVUK_NOTIFY_TEMPLATE_ID_NON_SJP_PDF_EXCEL environment variable is not set");
    }
    return GOVUK_NOTIFY_TEMPLATE_ID_NON_SJP_PDF_EXCEL;
  }
