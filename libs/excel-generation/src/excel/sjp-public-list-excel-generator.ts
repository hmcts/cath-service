...
import { getSjpPublicListHeaders } from "./excel-headers.js";
import { CELL_BORDER, DATA_ALIGNMENT, DATA_FONT, HEADER_ALIGNMENT, HEADER_FILL, HEADER_FONT } from "./excel-styles.js";

export async function generateSjpPublicListExcel(json: SjpJson, locale: "en" | "cy" = "en"): Promise<Buffer> {
  const cases = extractPublicCases(json);
  const headers = getSjpPublicListHeaders(locale);
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet("SJP Public List");

  worksheet.columns = [
    { header: headers.name, key: "name", width: 30 },
    { header: headers.postcode, key: "postcode", width: 15 },
    { header: headers.offence, key: "offence", width: 40 },
    { header: headers.prosecutor, key: "prosecutor", width: 30 }
  ];
