...
export const GET = async (req: Request, res: Response) => {
  ...
  const artefactId = req.query.id as string;

  if (!artefactId) {
    return res.status(400).render("view-cause-list-error", {
      en,
      cy,
      title: t.errorTitle,
  ...
  try {
    ...
    if (!artefact) {
      return res.status(404).render("view-cause-list-error", {
        en,
        cy,
        title: t.errorTitle,
        message: t.errorMessage
    ...
    return res.status(500).render("view-cause-list-error", {
      en,
      cy,
      title: t.errorTitle,
      message: t.errorMessage
