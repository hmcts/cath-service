...
Migrated from VIBE-297. Original metadata: Status: New, Priority: 3-Medium, Issue Type: Story, Created: 12/1/2025, Updated: 12/1/2025, Original Labels: cath
...
### Comment by hmctsclaudecode[bot] on 2026-02-23
## 1. User Story
...
I've created a technical plan in the `feature/292-language-toggle-service-navigation` branch (`docs/tickets/292/plan.md`). The plan is ready to implement with no blockers, but I'd like to confirm the following before coding:

**1. Toggle link visible text**
The current phase banner uses just `"Cymraeg"` / `"English"` (single-word). The plan reuses these same values in the service navigation. Is single-word acceptable, or does design want something like `"Welsh (Cymraeg)"` as the visible link text?

**2. Toggle position within the navigation end slot**
The plan places the toggle **before** Sign in/Sign out (reading left-to-right: `[Cymraeg] [Sign in]`). The wireframe in the spec shows `[Sign in] [Cymraeg]`, which would make the toggle the **last** item. Which order is correct?

**3. `language.switchPhaseBanner` key rename**
The plan renames `language.switchPhaseBanner` → `language.switch` in the shared locale files. A grep confirms only `phase-banner-content.njk` uses the old key. Are there any external consumers (e.g., automated screenshot tools, config files outside the repo) that reference `switchPhaseBanner` directly?

**4. Welsh aria-label translation**
The spec notes `language.switch` aria-label for Welsh is `[WELSH TRANSLATION REQUIRED: "Change language to English"]`. The plan uses `"Switch to English (Saesneg)"` as a placeholder. Please provide the confirmed Welsh translation for this string.
