...
### Comment by hmctsclaudecode on 2026-07-29T14:25:07Z

## Technical plan for #742

Plan committed to `docs/tickets/742/` on branch [`feature/742-sjp-publishing-advisory`](https://github.com/hmcts/cath-service/tree/feature/742-sjp-publishing-advisory/docs/tickets/742).

**Approach:** content-only. Two locale keys (`sjpAdvisoryPrefix`, `sjpAdvisoryMessage`) in the page's `en.ts`/`cy.ts`, one boolean render flag (`isSjpVenue`), and one conditional `<p class="govuk-body">` block in `summary-of-publications/index.njk`. The template's existing `cautionMessage` slot already sits exactly where the AC wants the advisory — after the FaCT paragraph and above the branch that chooses between "Select the list you want to view…" and "Sorry, no lists found for this court" — so **a single insertion satisfies both AC placements** with no duplicated markup. "Please note:" is bolded with a real `<strong>` element rather than HTML in the content string, so nothing needs `| safe`.

Two corrections to the earlier `@spec` comment on this issue, both verified against the code:

- It proposed extending `e2e-tests/tests/summary-of-publications.spec.ts`. That file creates its own dynamic test locations and never visits `locationId=9`, so the advisory would never render there. The SJP venue journeys are in `e2e-tests/tests/publication-authorisation.spec.ts` — the existing unauthenticated journey there will be extended instead.
- It proposed a new `libs/location/src/sjp-location.ts` constant plus a refactor of `view-option`. Dropped on YAGNI grounds: only one site needs the numeric *comparison*, and three of the four other `9` occurrences are `.njk` hrefs and test assertions that can't import a TS constant, so the new lib module wouldn't remove any duplication. A module-scope const in the controller is enough.

Worth noting on ID safety: `Location.locationId` is `Int @id` with **no** `@default(autoincrement())` and is explicitly assigned in `location-data.ts`, seeded via `ON CONFLICT (location_id)`. So `9` is stable across environments — the CLAUDE.md "never use numeric IDs" rule is specific to `ListType.id` and doesn't bite here.

---

### Clarifications needed

1. **Should the advisory show all year round, or only during BST / only before 10:15am?** The problem statement cites BST as the trigger, but the AC states the message unconditionally with no time or date condition. **Recommendation: always-on.** It's accurate year-round (lists are still published up until 10:15am in GMT), and conditional display would mean the page reads differently for two users minutes apart — harder to support, and it adds timezone logic for no user benefit. The plan assumes always-on; please confirm, as a time-conditional variant is a materially different build.

2. **Is `location_metadata.cautionMessage` currently set for location 9 on STG?** If it is, the page will show two stacked advisory blocks. The AC placement would technically be met but the page would read badly. Could someone with STG access check and clear/reconcile it before release?

3. **Does "the SJP venue" mean only `locationId: 9` (Single Justice Procedure), or every venue in the Magistrates Court sub-jurisdiction (`subJurisdictionId: 7`)?** That sub-jurisdiction also covers e.g. Birmingham Magistrates' Court (`locationId: 12`). The plan assumes location 9 only.

4. **Has the Welsh copy been through Welsh Language Unit assurance?** It was supplied in the ticket rather than via the usual translation route. Two specifics: the Welsh expands SJP to "Weithdrefn Un Ynad (SJP)" while the English leaves it unexpanded (I've treated that asymmetry as intentional and won't "correct" it), and the mutation in `gwrandawiadau'r`. Also — is bolding `Sylwer:` the right Welsh equivalent of bolding `Please note:`?

5. **Is the 10:15am cut-off expected to change, or are similar advisories wanted for other venues?** If so, the right long-term shape is a generic per-location advisory field in `location_metadata` with English and Welsh columns, managed at `/location-metadata-manage`. That's larger than this ticket — I'd raise it as a follow-up rather than pre-build it. But worth knowing now.

One thing outside this ticket's scope but worth flagging: this advisory is a workaround for an upstream scheduling inconsistency. A message telling users the feed is late is weaker than the feed not being late — probably worth raising with the team owning the SJP trigger.
