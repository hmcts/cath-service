...
3. **`apps/web/src/app.ts`** — import `moduleRoot as insolvencyModuleRoot` from `.../config` and add to the `modulePaths` array (mirrors `rcjStandardModuleRoot` at `app.ts:33,132`). Pages are auto-discovered — no route registration needed. Add the module's `assets` to `apps/web/vite.config.ts` only if the lib ships assets (it need not).
   - **Converter side-effect import (easy to miss):** the RCJ-family converters are registered by a bare side-effect import in `apps/web/src/pages/(admin)/non-strategic-upload-summary/index.ts` (see the existing `import "@hmcts/administrative-court-daily-cause-list";` / `"@hmcts/rcj-standard-daily-cause-list";` lines) — that page performs the Excel→JSON conversion. Add `import "@hmcts/insolvency-and-companies-court-chd-daily-cause-list";` there so `registerConverterByName` runs. Without this the upload conversion for this list type will fail at runtime even though everything else is wired.
